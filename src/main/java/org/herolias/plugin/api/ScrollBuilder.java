...
public class ScrollBuilder {
    ...
    ScrollBuilder(int level, @Nullable EnchantmentBuilder parent) {
        if (level < 1) throw new IllegalArgumentException("Scroll level must be >= 1, got: " + level);
        this.level = level;
        this.parent = parent;
    }
    ...
    public ScrollBuilder craftingTier(int tier) {
        if (tier < 1 || tier > 4) throw new IllegalArgumentException("Crafting tier must be 1-4, got: " + tier);
     ...
     *
     * @param modelPath Path relative to the mod's assets (e.g. "Items/MyScroll.blockymodel")
     */
     ...
     *
     * @param texturePath Path relative to the mod's assets (e.g. "Items/MyScroll.png")
     */
