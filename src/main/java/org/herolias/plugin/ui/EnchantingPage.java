...
public class EnchantingPage extends InteractiveCustomUIPage<EnchantingPageEventData> {
    ...
    public void handleDataEvent(@Nonnull Ref<EntityStore> ref, @Nonnull Store<EntityStore> store,
        ...
        if (data.tabSwitch != null) {
            ...
            this.playerRef.sendMessage(Message.raw("Click here to join the Simple Enchantments Discord! ").insert(Message.raw("[Discord Link]").link("https://discord.gg/7XQAnUfQfM").color("#5865F2")).bold(true));
            return;
    ...
    private void buildTabContent(@Nonnull UICommandBuilder commandBuilder, @Nonnull UIEventBuilder eventBuilder, boolean rebuildSidebar) {
    ...
    private void buildWalkthroughTab(@Nonnull UICommandBuilder commandBuilder, @Nonnull UIEventBuilder eventBuilder, boolean rebuildSidebar) {
        if (rebuildSidebar) {
        ...
        for (int j = 0; j < enchantments.length; j++) {
            ...
            if (rebuildSidebar) {
                commandBuilder.append("#WalkthroughSidebarContainer #EnchantmentList", "Pages/EnchantingWalkthroughSidebarItem.ui");
            }
    ...
    private void setupWalkthroughTranslations(@Nonnull UICommandBuilder commandBuilder, String lang, int page, EnchantmentType[] enchantments) {
        if (page < 6) {
            switch (page) {
                case 0 -> {
                    ...
                    commandBuilder.set("#ContentArea[0] #DiscordBtn.TextSpans", Message.raw("Join Discord").color("#ffffff"));
                }
                case 1 -> {
